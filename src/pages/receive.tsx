...
function ReceiveAssets() {
    ...
    const qrCodeUrl = address
        ? `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${address}&bgcolor=ffffff&color=2563eb&margin=10`
        : "";
        ...
        <div className="min-h-full bg-black flex flex-col p-6 animate-fade-in-up">
            ...
            <div className="flex-1 flex flex-col items-center">
                <p className="text-gray-400 text-center text-xs mb-8 px-4 leading-relaxed">
                    Scan this QR code or copy the address below to receive tokens on the <span className="text-blue-400 font-bold">Sepolia Network</span>.
                </p>

                {/* QR Code Container */}
                <div className="relative p-4 rounded-3xl bg-white mb-8 shadow-2xl shadow-blue-600/30 group overflow-hidden">
                ...
                <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 mb-8">
                    <label className="block text-[10px] uppercase tracking-[0.2em] font-black text-blue-500 mb-3">Your Wallet Address</label>
                    <p className="text-white font-mono break-all text-xs leading-relaxed mb-5 bg-white/5 p-3 rounded-lg border border-white/5">
                        {address || "Loading..."}
                    </p>
                    <button
                        onClick={handleCopy}
                ...
                <div className="w-full p-4 rounded-2xl bg-orange-500/5 border border-orange-500/10 flex gap-4">
                    ...
                    <p className="text-[11px] text-orange-200/60 leading-relaxed">
                        This address is on the <span className="text-orange-400 font-bold">Sepolia Testnet</span>. Do not send real ETH here.
                    </p>
                </div>
            </div>
            ...
            <button
                onClick={() => navigate("/dashboard")}
                className="mt-8 py-2 text-gray-500 font-bold text-xs hover:text-white transition-colors uppercase tracking-widest"
            >
                Return to Home
            </button>
        </div>
