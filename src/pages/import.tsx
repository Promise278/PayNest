        ...
        <div className="min-h-full bg-black flex flex-col p-6 animate-fade-in-up">
            ...
            <div className="flex items-center mb-8">
                <button
                    ...
                    className="p-2 -ml-2 rounded-full text-zinc-400 hover:text-white transition-colors"
                >
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                    </svg>
                </button>
                <h1 className="text-xl font-bold ml-2 text-white">Import Wallet</h1>
            </div>
            ...
            {step === 1 ? (
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-8">
                        {mnemonic.map((word, i) => (
                            <div key={i} className="relative">
                                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] text-zinc-600 font-bold">{i + 1}</span>
                                <input
                                    type="text"
                                    value={word}
                                    onChange={(e) => handleMnemonicChange(i, e.target.value)}
                                    className="w-full bg-zinc-900 border border-white/5 rounded-xl pl-6 pr-2 py-3 text-white text-xs focus:outline-none focus:border-white/30 transition-colors"
                                />
                            </div>
                        ))}
                    </div>
                    ...
                    <button
                        onClick={nextStep}
                        className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl transition-all shadow-lg shadow-indigo-500/20 mt-auto active:scale-[0.98]"
                    >
                        Continue
                    </button>
                </div>
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="space-y-4 mb-8">
                        <div>
                            <label className="block text-[10px] uppercase tracking-widest font-black text-zinc-500 mb-2">New Password</label>
                            <input
                                type="password"
                                value={password}
                                onChange={(e) => setPassword(e.target.value)}
                                placeholder="Min. 8 characters"
                                className="w-full bg-zinc-900 border border-white/5 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-white/30 transition-all"
                            />
                        </div>
                        <div>
                            <label className="block text-[10px] uppercase tracking-widest font-black text-zinc-500 mb-2">Confirm Password</label>
                            <input
                                type="password"
                    ...
                    <div className="p-4 rounded-2xl bg-zinc-900 border border-white/5 flex gap-3 mb-8">
