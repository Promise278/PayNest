...
function CreateWallet() {
    ...
    return (
      <div className="min-h-full bg-black flex flex-col p-6 animate-fade-in-up">
        <div className="flex items-center mb-8">
          <button onClick={() => setStep("phrase")} className="text-white hover:text-zinc-300 transition-colors p-2 -ml-2 rounded-full">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          </button>
          <h1 className="text-2xl font-bold ml-2 text-white">Secure Wallet</h1>
        </div>
        <div className="flex-1 space-y-6">
          <p className="text-zinc-400 text-sm">Create a password to encrypt your wallet. You will need this to unlock PayNest.</p>
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-zinc-500 uppercase mb-2">Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 characters"
                className="w-full bg-zinc-900 border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-white/30 transition-colors" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-zinc-500 uppercase mb-2">Confirm Password</label>
  ...
  return (
    <div className="h-full w-full bg-black flex flex-col p-6 relative">
      <div className="flex items-center mb-8">
        <button onClick={() => navigate("/")} className="text-white hover:text-zinc-300 transition-colors p-2 -ml-2 rounded-full">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        </button>
      ...
      <div className="flex-1 flex flex-col items-center">
        <p className="text-zinc-400 text-center text-sm mb-6">Write down these 12 words in order and keep them somewhere safe. Anyone with this phrase can access your funds.</p>

        <div className="relative w-full mb-6">
          <div className="grid grid-cols-3 gap-3">
            {mnemonic.map((word, index) => (
              <div key={index} className={`bg-zinc-900 border border-white/5 rounded-lg p-2 text-center flex flex-col items-center transition-all duration-300 ${!isRevealed ? "blur-[6px] opacity-70" : "blur-0 opacity-100"}`}>
                <span className="text-xs text-zinc-600 mb-1">{index + 1}</span>
                <span className="text-white font-medium text-sm select-all">{word || "..."}</span>
              </div>
            ))}
          </div>
          {!isRevealed && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm rounded-xl border border-white/10">
              ...
              <button onClick={() => setIsRevealed(true)} className="px-6 py-2 bg-zinc-800 hover:bg-zinc-700 text-white text-sm font-semibold rounded-lg transition-colors border border-white/10 shadow-lg shadow-black/50">
                Reveal Seed Phrase
              </button>
            </div>
          )}
        ...
        <div className="w-full space-y-3 mt-auto mb-4">
          ...
          <button disabled={!hasCopied} onClick={handleWalletConfirm}
