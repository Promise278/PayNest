      ...
      <div className="min-h-full bg-black flex flex-col p-6 animate-fade-in-up">
        ...
        <div className="flex-1 space-y-6">
          <p className="text-gray-400 text-sm">
            Create a password to encrypt your wallet. You will need this to
            unlock PayNest.
          </p>
        ...
        <div className="relative w-full mb-6">
          <div className="grid grid-cols-3 gap-3">
            {walletData.mnemonicWords.map((word, index) => (
              <div
                key={index}
                className={`bg-white/5 border border-white/10 rounded-lg p-2 text-center flex flex-col items-center transition-all duration-300 ${
                  !isRevealed ? "blur-[6px] opacity-70" : "blur-0 opacity-100"
                }`}
              >
                <span className="text-xs text-blue-500/70 mb-1">
                  {index + 1}
                </span>
                <span className="text-white font-medium text-sm select-all">
                  {word || "..."}
                </span>
              </div>
            ))}
